...
For ad-style edits where every frame matters, generate per-scene VO first and anchor every visual to known absolute timestamps. This eliminates timing drift entirely. See **AGENTS.md → Video Timing → Audio-Anchored Timelines** for the full pattern. The short version:
...
- Audio-anchored timelines: `AGENTS.md → Video Timing → Audio-Anchored Timelines`
