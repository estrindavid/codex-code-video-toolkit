...
This file provides guidance to Codex (https://developers.openai.com/codex) when working with this video production toolkit.

## Overview

**codex-code-video-toolkit** is an AI-native video production workspace. It provides Codex with the skills, workflow prompts, and tools to create professional videos from concept to final render.
...
`_internal/toolkit-registry.json` is the canonical source for all skills, workflow entries, tools, templates, components, transitions, and cloud endpoints — including their paths, status, options, presets, and env vars. Consult it for structured data. This file focuses on **workflow guidance, patterns, and knowledge** that the registry can't capture.
...
> **Note:** Codex discovers repo-scoped skills from `.agents/skills`. Use `$skill-name` in the CLI; in the Codex app, enabled skills may also appear in the command list.
...
```bash
cd /path/to/codex-code-video-toolkit && python3 tools/upscale.py ...
```
...
8 scene presets: `corporate-bg`, `upbeat-tech`, `ambient`, `dramatic`, `tension`, `hopeful`, `cta`, `lofi`. See `.agents/skills/acestep/` for prompt engineering patterns and video production integration guide.
...
9. **Iterate** - Adjust timing, content, styling with Codex
...
- Lives in `projects/`
- Each project has `project.json` (machine-readable state) and auto-generated `AGENTS.md`
